...
{
    public partial class SaidaDeItens : Form
    {
        FormCollection fc = Application.OpenForms;

        Frames.Success SuccessForm = new Frames.Success();
        Frames.Erro ErrorForm = new Frames.Erro();
        Frames.DeleteSelected2 DeleteSelectedForm = new Frames.DeleteSelected2("SaidaItem", ProductNameString);
        ...
        private void SaidaDeItens_Load(object sender, EventArgs e)
        {
            // TODO: esta linha de código carrega dados na tabela 'allSaidaItemData.SaidaDeItens'. Você pode movê-la ou removê-la conforme necessário.
            this.saidaDeItensTableAdapter1.Fill(this.allSaidaItemData.SaidaDeItens);
            // All saida de itens data
            this.saidaDeItensTableAdapter1.Fill(this.allSaidaItemData.SaidaDeItens);

            // Saida de itens data
            this.saidaDeItensTableAdapter.Fill(this.saidaItemData.SaidaDeItens);
            ...
            if (IsDarkModeEnabled)
            {
                ...
                FilterResponsavelBtn.Image = Properties.Resources.responsavel_branco;
            ...
            else
            {
                ...
                FilterResponsavelBtn.Image = Properties.Resources.responsavel_preto;
            ...
            if (saidaItemData.SaidaDeItens.Count == 0)
            {
            ...
            if (TodosFornecedores.Checked)
                ResponsavelOptions.Size = new Size(220, 64);
            else
                ResponsavelOptions.Size = new Size(220, 154);

            PagesCount = Convert.ToInt32(Math.Ceiling(saidaItemData.SaidaDeItens.Count * 1.0 / PageRows));
            CurrentPage = 0;
            RefreshPagination();
            ReloadPage();
        ...
        private void SaidaDeItens_Click(object sender, EventArgs e)
        {
            HideFrames();
        }
        ...
        {
            var query = from campos in saidaItemData.SaidaDeItens
                        select new
                        {
                            campos.CODIGO,
                            ...
                            campos.QNTDRETIRADA,
                            campos.VALORTOTAL,
                            campos.ORDEMDESERVICO,
                            campos.ORDEMDEPRODUCAO,
                            campos.VENDANUMERO,
            ...
            saidaDeItensBindingSource.DataSource = query.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
        ...
        public void UpdateGrid()
        {
            ...
            {
                con.Open();
                OleDbDataAdapter da = new OleDbDataAdapter("SELECT * FROM SaidaDeItens ORDER BY [" + SortedByItem + "] DESC", con);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    // Itens data
                    saidaDeItensBindingSource.DataSource = customrows;
                    saidaDeItensTableAdapter.Fill(saidaItemData.SaidaDeItens);

                    // All itens data
                    saidaDeItensBindingSource1.DataSource = customrows;
                    saidaDeItensTableAdapter1.Fill(allSaidaItemData.SaidaDeItens);
        ...
        private void SpecificDateFunction(DateTime data1, DateTime data2)
        {
            ...
            saidaDeItensBindingSource.DataSource = query.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
            saidaDeItensBindingSource1.DataSource = query;
            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
        ...
        private void SortItens(object sender, EventArgs e)
        {
            ...
            else if (RetiradaSort.Checked)
            {
                OrderByColumn("Retirada");
                SortedByItem = "Retirada";
            }
            ...
            else if (OrdemDeServicoSort.Checked)
            {
                OrderByColumn("OrdemServico");
                SortedByItem = "OrdemServico";
            }
        ...
        private void OrderByColumn(string Campo)
        {
            if (DataFiltered)
            {
                if (SortedByOrder == "Descending")
                {
                    ...
                    {
                        case "Codigo":
                            var CodigoQuery = from campos in saidaItemData.SaidaDeItens
                            ...
                            saidaDeItensBindingSource.DataSource = CodigoQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = CodigoQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;

                        case "NumeroFabricante":
                            var FabricanteQuery = from campos in saidaItemData.SaidaDeItens
                                                orderby campos.NUMEROFABRICANTE descending
                                                where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))

                                                select campos;

                            saidaDeItensBindingSource.DataSource = FabricanteQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = FabricanteQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;

                        case "Produto":
                            var ProdutoQuery = from campos in saidaItemData.SaidaDeItens
                                            orderby campos.PRODUTO descending
                                            where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))

                                            select campos;

                            saidaDeItensBindingSource.DataSource = ProdutoQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = ProdutoQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;

                        case "Retirada":
                            var RetiradaQuery = from campos in saidaItemData.SaidaDeItens
                                           orderby campos.QNTDRETIRADA descending
                                           where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))

                                           select campos;

                            saidaDeItensBindingSource.DataSource = RetiradaQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = RetiradaQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;

                        case "NotaFiscal":
                            var NotaQuery = from campos in saidaItemData.SaidaDeItens
                                              orderby campos.NUMERONOTAFISCAL descending
                                              where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))

                                              select campos;

                            saidaDeItensBindingSource.DataSource = NotaQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = NotaQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;

                        case "ValorTotal":
                            var ValorQuery = from campos in saidaItemData.SaidaDeItens
                                                orderby campos.VALORTOTAL descending
                                                where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))

                                                select campos;

                            saidaDeItensBindingSource.DataSource = ValorQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = ValorQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;

                        case "OrdemServico":
                            var OrdemServicoQuery = from campos in saidaItemData.SaidaDeItens
                                                   orderby campos.ORDEMDESERVICO descending
                                                   where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))

                                                   select campos;

                            saidaDeItensBindingSource.DataSource = OrdemServicoQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = OrdemServicoQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;

                        case "Responsavel":
                            var ResponsavelQuery = from campos in saidaItemData.SaidaDeItens
                                               orderby campos.FUNCIONARIORESPONSAVEL descending
                                               where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))

                                               select campos;

                            saidaDeItensBindingSource.DataSource = ResponsavelQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = ResponsavelQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;

                        case "Observacoes":
                            var ObservacoesQuery = from campos in saidaItemData.SaidaDeItens
                                               orderby campos.OBSERVACOES descending
                                               where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))

                                               select campos;

                            saidaDeItensBindingSource.DataSource = ObservacoesQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = ObservacoesQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;

                        case "Entrada":
                            var EntradaQuery = from campos in saidaItemData.SaidaDeItens
                                               orderby campos.DATA >= campos.DATA
                                               where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))

                                               select campos;

                            saidaDeItensBindingSource.DataSource = EntradaQuery.Skip(PageRows * CurrentPage).Take(PageRows).ToList();
                            saidaDeItensBindingSource1.DataSource = EntradaQuery;
                            PagesCount = Convert.ToInt32(Math.Ceiling(AllDataGrid.RowCount * 1.0 / PageRows));
                            break;
                    }
                }
                ...
                else if (SortedByOrder == "Ascending")
                {
                    switch (Campo)
                    {
                        ...
                        case "Retirada":
                            var RetiradaQuery = from campos in saidaItemData.SaidaDeItens
                                                 orderby campos.QNTDRETIRADA ascending
                                                 where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))
                        ...
                        case "OrdemServico":
                            var OrdemServicoQuery = from campos in saidaItemData.SaidaDeItens
                                                  orderby campos.ORDEMDESERVICO ascending
                                                  where campos.DATA >= Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)) && campos.DATA <= Convert.ToDateTime(DateTime.Today.ToString("dd-MM-yyyy"))
            ...
            if (SortedByOrder == "Ascending")
            {
                if (All.Checked)
                {
                    foreach (DataGridViewColumn column in EntradaGrid.Columns)
                    {
                        ...
                        {
                            EntradaGrid.Columns[3].HeaderText = "QNTD. RETIRADA ↑";
                        ...
                        {
                            EntradaGrid.Columns[6].HeaderText = "ORD. SERVIÇO ↑";
                            EntradaGrid.Columns[6].ToolTipText = "Classificado: Menor para a maior";
                        }
                        else if (ResponsavelSort.Checked)
                        {
                ...
                else if (OrdemDeServico.Checked)
                {
                    if (OrdemDeServicoSort.Checked)
                    {
                        EntradaGrid.Columns[6].HeaderText = "ORD. SERVIÇO* ↑";
        ...
        private async void FornecedorChecked(object sender, EventArgs e)
        {
            if (TodosFornecedores.Checked != true)
            {
                ...
                if (PesquisarResponsavel.Text != "")
                {
                    FilterFunction("FUNCIONARIORESPONSAVEL", PesquisarResponsavel.Text);
                }
        ...
        {
            DataView dv = saidaItemData.SaidaDeItens.DefaultView;

            if (DataFiltered)
            {
                if (All.Checked)
                {
                    dv.RowFilter = string.Format("convert (CODIGO, 'System.String') LIKE '%" + Search.Text + "%' OR PRODUTO LIKE '%" + Search.Text + "%' OR NUMEROFABRICANTE LIKE '%" + Search.Text + "%' OR convert (QNTDRETIRADA, 'System.String') LIKE '%" + Search.Text + "%' \n" +
                        "OR convert (NUMERONOTAFISCAL, 'System.String') LIKE '%" + Search.Text + "%' OR VALORTOTAL LIKE '%" + Search.Text + "%' OR FUNCIONARIORESPONSAVEL LIKE '%" + Search.Text + "%' OR convert(ORDEMDESERVICO, 'System.String') LIKE '%" + Search.Text + "%' OR DATA LIKE '%" + Search.Text + "%' \n" +
                    ...
                    saidaDeItensBindingSource.DataSource = dv;
                    saidaDeItensBindingSource1.DataSource = dv;
                }
                ...
                {
                    dv.RowFilter = "convert(QNTDRETIRADA, 'System.String') LIKE '%" + Search.Text + "%' " +
                        "AND ENTRADA >= '" + Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)).ToString("dd-MM-yyyy") + "' AND ENTRADA <= '" + Convert.ToDateTime(DateTime.Today).ToString("dd-MM-yyyy") + "'";
                    saidaDeItensBindingSource.DataSource = dv;
                    saidaDeItensBindingSource1.DataSource = dv;
                }
                ...
                {
                    dv.RowFilter = "convert(ORDEMDESERVICO, 'System.String') LIKE '%" + Search.Text + "%' " +
                        "AND ENTRADA >= '" + Convert.ToDateTime(DateTime.Now.AddDays(-FilterByDays)).ToString("dd-MM-yyyy") + "' AND ENTRADA <= '" + Convert.ToDateTime(DateTime.Today).ToString("dd-MM-yyyy") + "'";
                    saidaDeItensBindingSource.DataSource = dv;
                    saidaDeItensBindingSource1.DataSource = dv;
                }
        ...
        private void HideFrames()
        {
            ...
            if (ResponsavelOptions.Visible)
            {
                if (RemoverFiltros.Visible)
                    ResponsavelOptions.Location = new Point(5512, ResponsavelOptions.Location.Y);
                else
                    ResponsavelOptions.Location = new Point(5512, ResponsavelOptions.Location.Y);
            }
        ...
        private void HideNewSaidaItens()
        {
            EntradaGrid.Visible = true;
            Separator2.Visible = true;
            toolStripPaging.Visible = true;
        ...
        private void AddControlsToList()
        {
            ...
            {
                FilterItens, SearchOptions, DataOptions, ResponsavelOptions, SpecificDate,
            ...
            {
                FilterPesquisaBtn, FilterResponsavelBtn, FilterDataBtn, DataEspecifica,
            ...
            {
                All, Codigo, Produto, NumeroFabricante, Retirada, NotaFiscal, Valor, Responsavel, Data, OrdemDeServico,

                TodosFornecedores, FornecedorEspecifico,

                TodoPeriodo, Hoje, Semana, Mes, Ano, AllColumns, Principais, CodigoSort, ProdutoSort, 

                NumeroFabricanteSort, RetiradaSort, NotaFiscalSort, ValorSort, ResponsavelSort, DataSort, OrdemDeServicoSort
            };
        ...
        private void SetColor()
        {
            ...
            PesquisarResponsavel.FillColor = ThemeManager.SearchBoxFillColor2;
            PesquisarResponsavel.ForeColor = ThemeManager.SearchBoxForeColor;
            PesquisarResponsavel.BorderColor = ThemeManager.SeparatorAndBorderColor;
            PesquisarResponsavel.HoverState.BorderColor = ThemeManager.SearchBoxHoverBorderColor;
            PesquisarResponsavel.PlaceholderForeColor = ThemeManager.SearchBoxPlaceholderColor;
            PesquisarResponsavel.FocusedState.BorderColor = ThemeManager.SearchBoxFocusedBorderColor;
            PesquisarResponsavel.FocusedState.ForeColor = ThemeManager.SearchBoxForeColor;
            ...
            NovaSaida.BackColor = ThemeManager.FormBackColor;
            NovaSaida.FillColor = ThemeManager.FullRedButtonColor;
            NovaSaida.BorderColor = ThemeManager.FullRedButtonColor;
            NovaSaida.HoverState.FillColor = ThemeManager.FullRedButtonHoverColor;
            NovaSaida.HoverState.BorderColor = ThemeManager.FullRedButtonHoverColor;
            NovaSaida.CheckedState.FillColor = ThemeManager.FullRedButtonCheckedColor;
            NovaSaida.CheckedState.BorderColor = ThemeManager.FullRedButtonCheckedColor;
        ...
        private void NovoItem_Click(object sender, EventArgs e)
        {
            ...
            if (NovaSaidaOpen != true)
            {
                NovaSaidaOpen = true;

                ThreadStart ts = new ThreadStart(() =>
                {
                    DarkBackground(new Frames.NovaSaida());
                });
        ...
        {
            if (e.ColumnIndex == EntradaGrid.Columns[13].Index)
            {
        ...
        private async void FilterBtn_Click(object sender, EventArgs e)
        {
            if (FilterItens.Visible)
            {
                ...
                ResponsavelOptions.Visible = false;
                ResponsavelOptions.Location = new Point(512, ResponsavelOptions.Location.Y);
        ...
        private async void FilterFornecedoresBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (AnimateButtonsAndPanels)
                {
                    ...
                    {
                        await TaskDelay(10);
                        ResponsavelOptions.Location = new Point(ResponsavelOptions.Location.X + 1, ResponsavelOptions.Location.Y);
                        ResponsavelOptions.Visible = true;
                    }
                }
                else
                {
                    ResponsavelOptions.Location = new Point(ResponsavelOptions.Location.X + 6, ResponsavelOptions.Location.Y);
                    ResponsavelOptions.Visible = true;
                }
            }
        }
        ...
        private void CustomFornecedorBtn_Click(object sender, EventArgs e)
        {
            ...
            OleDbCommand cmd = new OleDbCommand("SELECT * FROM SaidaDeItens WHERE FUNCIONARESPONSAVEL = '" + PesquisarResponsavel.Text + "'", con);
            OleDbDataAdapter adapter = new OleDbDataAdapter(cmd);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                // All produtos data
                saidaDeItensBindingSource1.DataSource = customrows2;
                saidaDeItensTableAdapter1.Fill(allSaidaItemData.SaidaDeItens);
        ...
        private async void ExportCurrentExcel_Click(object sender, EventArgs e)
        {
            ...
            ExportExcel.FileName = "Planilha de saída de itens";
            ...
            if (ExportExcel.ShowDialog() != DialogResult.Cancel)
            {
                ...
                if (Principais.Checked)
                {
                    ...
                    for (int i = 1; i < EntradaGrid.Columns.Count; i++)
                    {
                        if (SortedByOrder == "Ascending")
                        {
                            ...
                            ExcelApp.Cells[9] = EntradaGrid.Columns[9].HeaderText.Replace("↑", "");
                        }
                    ...
                    for (int i = 0; i < EntradaGrid.Rows.Count; i++)
                    {
                        for (int j = 1; j < EntradaGrid.Columns.Count; j++)
                        {
                            ...
                            ExcelApp.Cells[i + 2, 9] = EntradaGrid.Rows[i].Cells[9].Value.ToString();
                        }
                    }
                }
                ...
                else if (AllColumns.Checked)
                {
                    ...
                    ExcelApp.Columns[12].ColumnWidth = 15;
                    ExcelApp.Columns[13].ColumnWidth = 20;
                    ...
                    for (int i = 1; i < EntradaGrid.Columns.Count; i++)
                    {
                        if (SortedByOrder == "Ascending")
                        {
                            ...
                            ExcelApp.Cells[12] = EntradaGrid.Columns[11].HeaderText.Replace("↑", "");
                            ExcelApp.Cells[13] = EntradaGrid.Columns[12].HeaderText.Replace("↑", "");
                        }
        ...
        private async void ExportAllExcel_Click(object sender, EventArgs e)
        {
            ExportExcel.InitialDirectory = "";
            ExportExcel.Title = "Salvar como arquivo Excel";
            ExportExcel.FileName = "Planilha geral de saída de itens";
            ExportExcel.Filter = "Arquivo Excel|*.xlsx";
                ...
                else if (AllColumns.Checked)
                {
                    ...
                    for (int i = 1; i < AllDataGrid.Columns.Count; i++)
                    {
                        if (SortedByOrder == "Ascending")
                        {
                            ...
                            ExcelApp.Cells[12] = AllDataGrid.Columns[11].HeaderText.Replace("↑", "");
                            ExcelApp.Cells[13] = AllDataGrid.Columns[12].HeaderText.Replace("↑", "");
                        }
        ...
        private async void ExportCurrentPdf_Click(object sender, EventArgs e)
        {
            if (EntradaGrid.Rows.Count > 0)
            {
                ...
                sfd.FileName = "Relatório de saída de itens.pdf";
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (!fileError)
                    {
                        if (Principais.Checked)
                        {
                            try
                            {
                                PdfPTable pdfTable = new PdfPTable(EntradaGrid.Columns.Count - 5);
                                ...
                                foreach (DataGridViewColumn column in EntradaGrid.Columns)
                                {
                                    if (column.Index != 8 && column.Index != 10 && column.Index != 11 && column.Index != 12 && column.Index != 13)
                                    {
                                ...
                                foreach (DataGridViewRow row in EntradaGrid.Rows)
                                {
                                    ...
                                    {
                                        if (cell.ColumnIndex != 8 && cell.ColumnIndex != 10 && cell.ColumnIndex != 11 && cell.ColumnIndex != 12 && cell.ColumnIndex != 13)
                                ...
                                using (FileStream stream = new FileStream(sfd.FileName, FileMode.Create))
                                {
                                    ...
                                    var Nome = new Paragraph("                           CLÍNICA CAR                                 " +
                                        "Relatório de saída de itens", NomeParagraph);
        ...
        private async void ExportAllPdf_Click(object sender, EventArgs e)
        {
            if (AllDataGrid.Rows.Count > 0)
            {
                SaveFileDialog sfd = new SaveFileDialog();
                sfd.Filter = "PDF (*.pdf)|*.pdf";
                sfd.FileName = "Relatório geral de saída de itens.pdf";
                bool fileError = false;
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (!fileError)
                    {
                        if (Principais.Checked)
                        {
                            try
                            {
                                PdfPTable pdfTable = new PdfPTable(AllDataGrid.Columns.Count - 5);
                                pdfTable.DefaultCell.Padding = 3;
                                pdfTable.WidthPercentage = 100;
                                pdfTable.HorizontalAlignment = Element.ALIGN_LEFT;
        ...
        private void Excluir_Click(object sender, EventArgs e)
        {
            foreach (Form frm in fc)
            {
                if (frm.Name != "DeleteSelected")
                    DeleteSaidaOpen = false;
                else
                    DeleteSaidaOpen = true;
            }

            if (DeleteSaidaOpen != true)
            {
                DeleteSaidaOpen = true;
        ...
        private void EntradaGrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (saidaItemData.SaidaDeItens.Count > 0)
            {
                if (EntradaGrid.CurrentCell.ColumnIndex == 13)
                {
                    ...
                    if (DeleteSaidaOpen != true)
                    {
                        ...
                        ThreadStart ts = new ThreadStart(() => {
                            DarkBackground(new Frames.DeleteSelected2("SaidaItem", ProductNameString));
                        });
        ...
        private void ListaNormal_Click(object sender, EventArgs e)
        {
            ...
            saidaDeItensBindingSource.DataSource = null;
            ReloadPage();
